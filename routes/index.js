...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        if (data.length < 1) { // 사용자 계정 존재 여부 검증.
            res.render('login.ejs', {error: 'Not registered account or wrong password'});
        } else {
...
router.post('/signup', async (req, res) => {
    ...
    try{
        ...
        if (data.length >= 1) { // 아이디와 이메일 중복 여부 검증
            res.render('signup.ejs', {error: 'Duplicate an username or email'});
        } else{     
            const hash_pass = crypto.createHash("sha256").update(body.password, "binary").digest("hex");
            if(!eamil_regex.test(body.email)) {
                res.render('signup.ejs', {error: 'Only use a dimigo email.'});
            }
            else{
                const token = uuid();
