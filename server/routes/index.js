        ...
        if (data.length >= 1) { // 아이디와 이메일 중복 여부 검증
            ...
            if(0) {
                ...
                const [data_] = await dbConnection.execute("INSERT INTO users (username, password, nickname, email, email_verify, token, solved, pts) values (?, ?, ?, ?, ?, ?, ?, 0)", 
                    [body.username, hash_pass, body.nickname, body.email, solved, token]
                );
    ...
    try{
        if(req.cookies['token'] !== undefined){
            ...
            if (challenge.length > 0) {
                J_solved = JSON.parse(user[0].solved) // by as3617
                ...
                if (check){
                    ...
                    J_solved.solved.push(challenge[0].id); // by as3617 : 사용자가 푼 문제 + 현재 푼 문제
                    ...
                    J_solver = JSON.parse(challenge[0].solver) // by as3617
                    J_solver.solver.push(user[0].id) // by as3617 : 문제를 해결한 모든 사용자 정리
